...
name: mj-writer
description: |
  MakerJackie 内容创作总入口。用于公众号文章、AI 教程、产品制作记录、Twitter 长推、选题生成、写作调研、语音转文章、文章审校、视频脚本口语化、演讲/分享稿优化等内容工作流。仅当用户目标是产出、整理、修改、规划或发布前打磨内容时使用。不要因为用户单独说“查一下”“最新信息”“设计”“部署”就触发。
---
...
需要作者介绍时，可以按语境使用以下短句；站内页面已有作者信息时不重复添加固定尾部：

```md
我是 Jackie，独立开发者，喜欢探索世界，也用 AI 做一些有趣的小产品。
更多文章、教程和作品：https://makerjackie.com
```

短进度和日常记录保留原有长度，不为凑长文补写经历、结果或观点。
