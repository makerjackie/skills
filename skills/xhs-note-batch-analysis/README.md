...
```bash
npx skills add makerjackie/skills --skill xhs-note-batch-analysis
```

## 快速使用

```text
Use $xhs-note-batch-analysis to collect this Xiaohongshu profile into Get笔记, then summarize from web_content:
...
```text
Use $xhs-note-batch-analysis with @chrome to collect this logged-in Xiaohongshu profile, import one sample to Get笔记, and show me the web_content quality.
```
