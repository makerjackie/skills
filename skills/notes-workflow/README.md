...
```bash
npx skills add makerjackie/skills --yes -g
```
...
- `~/.agents/notes-workflow/` for state storage

## State

Progress is tracked at `~/.agents/getnote-auto-process/state.json` so each run only processes new notes.
...
它像一个**语音笔记管家**，你只需要说 `/notes-workflow`，它就会：
