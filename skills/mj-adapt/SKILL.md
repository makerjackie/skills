...
### Step 5: 生成小红书内容图 (1242x1660px) - 版本 1 ： 保留文章原文，只做排版
- **视觉基调**：全篇统一使用白色背景。
- **分段与布局**：
    - 通常一个 H2 章节一张图。
    - **垂直对齐**：内容默认在画面中心或黄金分割点分布，底部保留自然留白，营造「杂志感」。
    - **组件复用**：使用卡片组件（方块边框）包裹核心 Step 或结论。
- **固定结尾页**：使用 `assets/xhs-ending-template.html`（已适配白色背景）。

### Step 6: 生成小红书内容图 (1242x1660px) - 版本 2 ： 根据内容自适应
- 基础要求同 step 5，不同之处在于
  - 允许修改原文，保留原意的基础上，可以适当增添或者修改文字，增加素材。以适应图片布局。
  - 允许使用更加丰富的排版元素，以向读者清晰展示文章的核心观点和细节。
  - 重点在于让文章更清晰易懂，更可视化。
- **固定结尾页**：使用 `assets/xhs-ending-template.html`。

### Step 7: 生成推特 (Twitter/X) 内容
根据文章深度自动选择模式：
