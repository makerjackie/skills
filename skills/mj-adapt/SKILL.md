...
- **样式规则**：
  ...
  - 每页必须包含正文容器：`<div data-role="content">...</div>`。页码、作者标识等页脚放入 `data-role="footer"` 或 `data-role="page-number"`，不要混入正文容器。
- **生成后必须自查**：
...
- **关键约束（必须遵守）**：
  ...
  - **正文内容必须使用内联样式**（与微信公众号 HTML 相同方式）；`<style>` 仅用于 reset、body 和 `.slide` 固定尺寸，**不得**引用 `xhs-base.css`。
  - 每页正文必须包在 `<div data-role="content">...</div>` 中；页脚、页码用 `data-role="footer"` 或 `data-role="page-number"`。
...
## 品牌与转化规范 (Branding & CTA)

### 1. 固定主次关系
- **Maker Jackie** 是人、信任和内容源头。用于作者署名、账号身份、个人故事、AI 实践和独立开发记录。
- **01MVP** 是产品、课程和付费转化入口。用于承接“我也想系统学习 AI 产品实战”的读者。
- 标准关系句：`01MVP 是 Maker Jackie 做的 AI 产品实战教程。`
- 不要把 `makerjackie.com / 01mvp.com` 固定并列塞进每篇内容。每篇文章只给一个主要动作：关注 Jackie，或者去 01mvp.com。

### 2. 场景化 CTA 适配
根据文章类型自动适配 CTA (Call to Action) 区域：
- **AI 教程 / 产品实战 / 独立开发复盘 / 工具教程**：主推 **01MVP**，轻带 Maker Jackie。
  - `{{CTA_LABEL}}`: `ABOUT 01MVP`
  - `{{CTA_CONTENT}}`: `01MVP 是 Maker Jackie 整理的 AI 产品实战手册，记录如何从一个想法出发，借助 AI 做出可上线的产品，并找到第一个真实用户。如果你正在用 AI 做产品，欢迎来 01mvp.com 从第一章开始。`
- **个人成长 / 思考 / 自媒体 / 生活记录**：主推 **Maker Jackie**，轻带 01MVP。
  - `{{CTA_LABEL}}`: `ABOUT MAKER JACKIE`
  - `{{CTA_CONTENT}}`: `我是 Jackie，独立开发者，01MVP 作者。这里记录 AI 实践、独立开发、个人成长和一些阶段性的答案。系统化的 AI 产品实战教程，我放在 01mvp.com。`
- **平台短内容**：公众号、小红书、X、B 站每条内容只保留一个转化口。技术教程给 01MVP；个人思考给 Maker Jackie。
...
- 2px 黑色边框包裹的全卡片
- 包含：Jackie 头像、简介
- 包含：一个主 CTA，根据文章意图指向 Maker Jackie 或 01MVP
- 包含：联系邮箱或账号 ID
```
...
- [ ] 公众号 HTML 和小红书 HTML 中没有未替换的 `{{PLACEHOLDER}}`。
