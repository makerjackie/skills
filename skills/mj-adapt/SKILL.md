...
将已完成的文章适配到多个发布平台，自动生成各平台所需的格式和素材（按用户指定的平台生成；仅网站入库时使用目标项目的发布 Skill）。
...
需要作者介绍时：
- `{{CTA_LABEL}}`: `ABOUT MAKERJACKIE`
- `{{CTA_CONTENT}}`: `我是 Jackie，独立开发者，喜欢探索世界，也用 AI 做一些有趣的小产品。更多文章、教程和作品：https://makerjackie.com`
- 根据平台限制选择站内关注、评论或可用链接；无须每篇都添加营销尾部。
...
- 2px 黑色边框包裹的全卡片
- 包含：Jackie 头像、简介
- 包含：至多一个主 CTA，按文章意图选择关注、继续阅读或相关作品
- 包含：联系邮箱或账号 ID
```
